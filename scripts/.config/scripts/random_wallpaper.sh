...
apply_wallpaper() {
    ...
    case "$XDG_SESSION_TYPE" in
        wayland)
            ...
            if command -v ruwall &>/dev/null; then
                ruwall -i "$wallpaper"
                echo "✅ Wallpaper aplicado com ruwall" >&2
                return 0
            fi
            ;;
        *)
            if command -v xwallpaper &>/dev/null; then
                ...
                echo "✅ Wallpaper aplicado com xwallpaper" >&2
                return 0
            fi
...
main() {
    ...
    # Regenera o tema de cores (pywal) e notifica
    wal -i "$SELECTED" >/dev/null 2>&1 || true

    # Reaplica as cores nos apps que cacheiam (polybar + dunst)
    "$HOME/.config/scripts/launch_polybar.sh" >/dev/null 2>&1 &
    if command -v dunstctl &>/dev/null && pgrep -x dunst >/dev/null 2>&1; then
        dunstctl reload 2>/dev/null || { pkill -x dunst 2>/dev/null || true; dunst >/dev/null 2>&1 & }
    else
        pkill -x dunst 2>/dev/null || true
        dunst >/dev/null 2>&1 &
    fi

    notify-send "Wallpaper" "$(basename "$SELECTED")" -i "$SELECTED" 2>/dev/null || true
    echo "✨ Concluído!" >&2
}
